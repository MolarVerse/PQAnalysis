...
def distance(pos1: Np1DNumberArray | Np2DNumberArray, pos2: Np1DNumberArray | Np2DNumberArray, cell: cell.Cell = cell.Cell()) -> PositiveReal | Np1DNumberArray | Np2DNumberArray:
    ...
    Notes
    -----
    In more detail this function returns the distances between all combinations of two positions in the position arrays 'pos1' and 'pos2'. If pos1 and pos2 are both 1D arrays, the distance between the two positions is returned. If pos1 is a 1D array and pos2 is a 2D array, the distances between the position in pos1 and all positions in pos2 are returned. If pos1 and pos2 are both 2D arrays, the distances between all combinations of positions in pos1 and pos2 are returned.

    The only requirement for the position arrays is that the last dimension has to be 3, which represents the x, y and z coordinates of the position(s).
