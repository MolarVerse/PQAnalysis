...
def write_improper_info(bonded_topology: BondedTopology, file: File) -> None:
    """
...
def write_shake_info(bonded_topology: BondedTopology, file: File) -> None:
    """
...
def get_angle_lines(bonded_topology: BondedTopology) -> List[str]:
    ...
    for angle in bonded_topology.angles:
        lines.append(
            f"{angle.index1} {angle.index2} {angle.index3} {angle.angle_type}"
        )
    ...
    for dihedral in bonded_topology.dihedrals:
        lines.append(
            f"{dihedral.index1} {dihedral.index2} {dihedral.index3} {
                dihedral.index4} {dihedral.dihedral_type}"
        )
    ...
    for improper in bonded_topology.impropers:
        lines.append(
            f"{improper.index1} {improper.index2} {improper.index3} {
                improper.index4} {improper.improper_type}"
        )
...
def get_shake_lines(bonded_topology: BondedTopology) -> List[str]:
    ...
    for bond in bonded_topology.shake_bonds:
        ...
        lines.append(
            f"{bond.index1} {bond.index2} {bond.equilibrium_distance} {linker}"
        )
