...
def read_analysis_table(filename: str) -> AnalysisTable:
    """
    Read native, CSV or TSV analysis data using content-based detection.
...
def _write_xvg_table(
    ...
    x_values, x_label = _select_xvg_x_axis(table, preset, x_field)
    y_values, y_label, legends = _select_xvg_y_axes(table, preset, y_fields)

    _write_xvg_header(table.schema, file, x_label, y_label, legends)
    _write_xvg_sets(file, x_values, y_values)
...
def _write_xvg_header(
    ...
    print(f'@    title "{_grace_quote(schema.title)}"', file=file)
    print(f'@    xaxis label "{_grace_quote(x_label)}"', file=file)
    print(f'@    yaxis label "{_grace_quote(y_label)}"', file=file)
    print("@    legend on", file=file)

    for index, legend in enumerate(legends):
        print(f'@    s{index} legend "{_grace_quote(legend)}"', file=file)
...
def _write_xvg_sets(
    ...
    for set_index, values in enumerate(y_values):
        for x_value, y_value in zip(x_values, values):
            print(
                f"{_format_number(x_value)} {_format_number(y_value)}",
                file=file,
            )
        if set_index != len(y_values) - 1:
            print("&", file=file)



def _select_xvg_x_axis(
    table: AnalysisTable,
    preset: AnalysisPlotPreset,
    x_field: str | None,
) -> tuple[np.ndarray, str]:
    selected_x = x_field if x_field is not None else preset.x_field
    if selected_x is None:
        return np.arange(1, table.data.shape[0] + 1), preset.x_label

    label = (
        preset.x_label if selected_x == preset.x_field else
        _column_label(table.schema, selected_x)
    )
    return table.column(selected_x), label



def _select_xvg_y_axes(
    table: AnalysisTable,
    preset: AnalysisPlotPreset,
    y_fields: Sequence[str] | None,
) -> tuple[tuple[np.ndarray, ...], str, tuple[str, ...]]:
    ...
    values = tuple(table.column(field) for field in selected_y)
    if selected_y == preset.y_fields:
        return values, preset.y_label, preset.legends

    label = (
        _column_label(table.schema, selected_y[0])
        if len(selected_y) == 1 else "Value"
    )
    legends = tuple(_column_label(table.schema, field) for field in selected_y)
    return values, label, legends
...
def _grace_quote(value: str) -> str:
