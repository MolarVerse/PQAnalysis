...
class TestPQAnalysisInputFileReader:
    @pytest.mark.parametrize(
        "example_dir",
        ["inputFileReader/PQAnalysis_input/"],
        indirect=False
    )
    ...
    def test__init__no_input_file(self):    
        with pytest.raises(PQFileNotFoundError) as exception:
            pqanalysis_input_file_reader  = PQAnalysisInputFileReader("no_input_file.in")
    ...
    def test_read(self, test_with_data_dir):
        ...
        assert pqanalysis_input_file_reader.raw_input_file == open("input_PQ.in",
            "r").read()
    ...
    def test_check_required_keys(self, test_with_data_dir):
        ...
        assert pqanalysis_input_file_reader.check_required_keys(required_keys) == None
    ...
    def test_check_known_keys(self, test_with_data_dir):
        ...
        known_keys = ["required_key1", "required_key2", "optional_key1", "optional_key2"]
        
        assert pqanalysis_input_file_reader.check_known_keys(known_keys) == None
    ...
    #         indirect=False
    # )
    # def test_check_known_keys_unknown_key(self,test_with_data_dir):
    #     pqanalysis_input_file_reader = PQAnalysisInputFileReader("input_unknown_keys.in")
    #     pqanalysis_input_file_reader.read()

    #     known_keys = ["known_key1", "known_key2"]
    #     # test warning 
    #     with pytest.warns(InputFileWarning) as record:
    #         pqanalysis_input_file_reader.check_known_keys(known_keys)
    #     assert str(record.list[0].message) == "Unknown keys were set in the input file! The known keys are: ['known_key1', 'known_key2']. They will be ignored!"
    ...
    def test_not_defined_optional_keys(self, test_with_data_dir):
        ...
        assert pqanalysis_input_file_reader.not_defined_optional_keys(optional_keys) == None
        assert pqanalysis_input_file_reader.dictionary["optional_key3"] == (None, "None", "None")
